...
router.post('/movement', async (req, res, next) => {
  try {
    ...
    const movement = await cash.addMovement(register_id, type, amount, reason);
...
router.get('/summary/:register_id', async (req, res, next) => {
  try {
    const summary = await cash.getSummary(req.params.register_id);
    res.json(summary);
  } catch (err) { next(err); }
});
...
router.post('/close', async (req, res, next) => {
  try {
    const { id, closing_amount, notes } = req.body;
    if (!id || closing_amount === undefined) {
      return res.status(400).json({ error: 'Faltan datos' });
    }
    const result = await cash.closeRegisterWithCount(id, closing_amount, notes);
    res.json(result);
  } catch (err) { next(err); }
});
