...
const insUser = db.prepare('INSERT OR IGNORE INTO users (username,password,name,role) VALUES (?,?,?,?)');
for (const u of users) {
  insUser.run(u.username, bcrypt.hashSync(u.password, 10), u.name, u.role);
}

const insCat = db.prepare('INSERT OR IGNORE INTO categories (name,section) VALUES (?,?)');
for (const c of categories) insCat.run(c.name, c.section);

const getCat = db.prepare('SELECT id FROM categories WHERE name=?');
const insProd = db.prepare(`INSERT OR IGNORE INTO products
  (barcode,name,category_id,cost,price,stock,stock_min,unit) VALUES (?,?,?,?,?,?,?,?)`);
for (const p of products) {
  const cat = getCat.get(p.cat);
  insProd.run(p.barcode, p.name, cat ? cat.id : null, p.cost, p.price, p.stock, 5, p.unit);
}

console.log('Seed listo. Usuarios: admin/admin123  cajero/cajero123');
process.exit(0);
